  ...
  <div id="ing-groups">
    <div class="row" v-for="(group, idx) in ingredientGroups" :key="idx">
      <fieldset class="col s12">
        ...
        <group-ingredients v-for="(ingredient, idx) in ingredients" :key="idx"
          v-if="isInGroup(group, ingredient.group)" :ingredient="ingredient"
          :idx="idx" :canDrag="!isModified && ingCountByGroup(group)>1"
          @onEvent="onEvent" @updated="$emit('updated')" />
        <div v-if="group == 'default' && !ingCountByGroup(group)"
          class="tip-wrapper centered mb-md">
          <p class="tip">Here you can:<br>&bull; Add a new "named" Ingredients
            Group
        ...
        <div class="row">
          ...
          <template v-else>
            <div class="col s12 center-align">
              <button class="btn btn-sm grey lighten-5" type="button"
                @click="onEvent('addItem', 'ingredients', undefined, {group: group})"
