...
const fetchRecipe = async ({commit, dispatch, getters, state}, recipe) => {
  ...
  recipe.method && isEmpty(recipe.methods) && (recipe.methods = recipe.method) && delete recipe.method;
...
const postRecipe = async ({commit, dispatch}, recipe) => {
  ...
  const resp = await axios.post(preAuthUrl, params, recipe).catch(err => {
    dispatch('handleError', {
      ...
      error: (err.response && `Error ${err.response.status}: ${err.response.statusText}`) || err,
    });
  });
  if (resp && resp.status === 200) {
    ...
    dispatch('handleError', {
      service: 'put:recipe',
      severity: 'error',
      error: (resp && `Error ${resp.status}: ${resp.statusText}`) || 'No Response',
    });
  }
};
...
export default {
  ...
  save(context) {
    ...
    (isUndefined(recipe.id) && postRecipe(context, recipe)) || putRecipe(context, recipe);
  },
};
